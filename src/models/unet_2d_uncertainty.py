...
class UNet2DUncertainty(UNet2DModel):
    def __init__(
        self,
        ...
        self.lower = nn.Conv2d(in_channels=in_channels, out_channels=1, kernel_size=3, padding=1)
        self.upper = nn.Conv2d(in_channels=in_channels, out_channels=1, kernel_size=3, padding=1)
    ...
    def forward(
        ...
        prediction = self.conv_out(sample)
        lower = self.lower(sample)
        upper = self.upper(sample)
        
        sample = torch.cat([lower, prediction, upper], dim=1)
