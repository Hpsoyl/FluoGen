...
def transform_condition(example, weight_dtype, device):
    ...
    tensor = torch.from_numpy(image).unsqueeze(0).unsqueeze(0).to(dtype=weight_dtype, device=device)
...
def log_validation(args, weight_dtype, device):
    ...
    masks_source_path = args.train_data_path
    image_source_path = masks_source_path.replace("masks", "images")

    output_dir = args.validation_output_dir
    os.makedirs(output_dir, exist_ok=True)
    
    valid_files = [f for f in os.listdir(masks_source_path) if f.endswith(".tif") and f.startswith("generated_")]
    total_images = len(valid_files)
    prompt = ""
    with tqdm(total=total_images, desc="Generating Images") as pbar:
        for tif_file in os.listdir(masks_source_path):
            if not (tif_file.endswith(".tif") and tif_file.startswith("generated_")):
                continue

            mask_path = os.path.join(masks_source_path, tif_file)
            mask = imageio.v2.imread(mask_path)
            mask = transform_condition(mask, weight_dtype, device)
            ...
            gen_image = np.squeeze((gen_image * 255).astype("uint8"))
            save_path = os.path.join(output_dir, f"{filename}.tif")
